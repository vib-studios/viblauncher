...
public partial class ModBrowserDialog : Window
{
    ...
    public ModBrowserDialog() => InitializeComponent();
        ...
        Window? owner = null)
    {
        ...
        if (provider is null)
        {
            ...
            await MessageDialog.ShowAsync("No mod provider is available.", reason, owner: owner).ConfigureAwait(true);
            return false;
        }
        ...
        dialog.Opened += async (_, _) => await dialog.InitialiseAsync().ConfigureAwait(true);
    ...
    private async void OnInstall(object? sender, RoutedEventArgs e)
    {
        ...
        if (report.UnresolvedDependencies.Count > 0)
        {
            await MessageDialog.ShowAsync(
                ...
                owner: this).ConfigureAwait(true);
            return;
        }
    ...
    private sealed class VersionRow
    {
        ...
        // The combo box's row template renders the item itself, so the label has
        // to come out of ToString the way SortChoice's and LoaderChoice's do.
