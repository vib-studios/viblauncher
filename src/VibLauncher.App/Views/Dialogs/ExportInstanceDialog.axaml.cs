...
public partial class ExportInstanceDialog : Window
{
        ...
        Window? owner = null)
    {
        ...
        {
            dialog.EmptyNotice.IsVisible = true;
            dialog.Contents.IsVisible = false;
        }

        var accepted = await MessageDialog.ShowOverAsync<bool>(dialog, owner).ConfigureAwait(true);

        return accepted ? dialog._contents.ToOptions() : null;
    }
