...
public partial class CreateInstanceDialog : Window
{
    ...
    public CreateInstanceDialog() => InitializeComponent();

    /// <summary>Runs the create flow. Returns the new instance, or <c>null</c> if cancelled.</summary>
    public static async Task<MinecraftInstance?> RunAsync(LauncherServices services, Window? owner = null)
    {
    ...
    private void SelectExisting(MinecraftInstance instance)
    {
        ...
        foreach (var item in LoaderBox.Items)
        {
            if (item is LoaderChoice choice && choice.Kind == instance.Loader)
            {
    ...
    private async Task<MinecraftInstance?> CreateAsync()
    {
        ...
        var instance = await _services.Instances
            .CreateAsync(new InstanceCreationRequest(InstanceName, version.Id, loader, loaderVersion))
            .ConfigureAwait(true);
    ...
    private void HideNotice() => NoticePanel.IsVisible = false;
    ...
    private string InstanceName => NameBox.Text?.Trim() ?? string.Empty;
